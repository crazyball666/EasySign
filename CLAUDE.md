...
- **Event-driven finite window:** an unexpected bound-connection drop, wake/app activation, an initial/restored usable network path, or the matching Bonjour peer appearing/changing can start recovery. One generation attempts at **0/2/5/10 seconds**; after four failures it enters `waitingForEvent` with no timer. A later valid recovery event starts a new generation. Repeated discovery of the same endpoint does not reset an active cycle.
- **One-way dial arbitration:** only the device with the **smaller deviceId** may dial; the larger device repairs its listener/advertisement and waits for inbound. An unknown/unsatisfied network path or missing current Bonjour endpoint never dials. Do not use a saved manual host to bypass arbitration: manual IP is only an explicit user Retry fallback.
- **Tokens and races:** every automatic token binds its generation/attempt to the expected `PeerRef` (deviceId + fingerprint) and current Bonjour discovery/recovery token. Each dial resolves the latest discovery snapshot, and TLS readiness rechecks both identity fields. User Connect/Retry cancels scheduled work and advances the generation, so stale automatic callbacks cannot replace or cancel it. If recovery is temporarily deferred because the service is busy, resuming uses the same token and **does not consume an attempt**; stale terminal callbacks may clean up only their own connection instance.
- **Wake/foreground infrastructure first (`onWokeOrActivated`):** repair the listener, reassert Bonjour when not in stealth mode, and restart discovery before requesting recovery, even while stale UI state still says connected/connecting. Bonjour reassertion has an independent **3-second debounce**; browser generations reject superseded callbacks, while peer appear/change tokens provide new recovery events.
- **Codeless reconnect and Retry:** automatic dials always use `pairingCode: nil` and TLS fingerprint pinning. After a successful user bind, Retry keeps the explicit peer or host/port target but becomes codeless; only the user can invoke the manual-IP fallback.
