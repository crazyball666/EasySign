...
设置与更新不迁入 Sidebar：设置继续作为 macOS `Settings` scene 打开，更新继续由现有菜单命令触发并呈现为主窗口 sheet。两者都使用相同的 GlassDesignSystem 组件重绘，但不改变窗口生命周期、路由或命令入口。

响应式优先级固定如下：

1. 宽度低于 1,180pt 时，页面右侧 context rail 收入可切换的 inspector popover；主画布占满 detail 区。
2. 设备页的设备列表在 detail 区低于 980pt 时从固定左列收为顶部横向 device picker；不会与全局 Sidebar 组成四列。
3. 全局 Sidebar 在窗口低于 820pt 时只保留图标 rail；低于 700pt 时维持系统 `NavigationSplitView` 的折叠行为。
4. 所有主操作保留在主画布，不能只存在于被收起的 rail 中。
...
“队列化进度卡”仅是现有单批 AFC 操作的视觉呈现：一次上传、下载、复制、移动或删除仍使用当前的单个批处理循环、冲突处理和取消语义。不新增并发队列、跨批重试、任务持久化或新的后台任务模型。
...
- 二维码使用中心画布与右侧说明/本次会话结果，突出扫描或分享的主要动作。会话结果仅复用当前 View 的生成状态和 `scanResults`；不新增持久化历史、隐私记录或清理规则。
