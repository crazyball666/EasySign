...
| 工具 | 描述 | 与现有联动 |
|---|---|---|
| **证书库** | 浏览/管理 p12 证书、过期提醒、删除 | 重签选证书时直接用 |
| **描述文件库** | 浏览/管理 mobileprovision、过期提醒、批量清理 | 重签选 profile 时直接用 |
| **崩溃符号化** | 拖入 .crash / .ips + dSYM 符号化 | 与重签的 build 号绑定 |
| **Plist 编辑** | 拖入 plist 可视化编辑，转 JSON/YAML | 与重签的 entitlements 编辑合并 |
| **IPA 元数据浏览器** | 把 QuickLook 扩展的能力搬到工具里 | 与重签的 IPAContentView 合并 |
| **设备日志** | 通过 `DeviceService` 拉 NSLog / Unified Log | Devices tab 升级 |
| **模拟器管理** | 列出/启动/清理 iOS Simulator | 独立 |

**第一批建议（Phase 6）选 1-2 个**：证书库 + 描述文件库是最直接的价值，门槛低，先验证"通过 ServiceHub 共享"的设计。
