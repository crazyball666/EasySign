...
struct IPAPreviewServiceTests {
    static func main() throws {
        ...
        assert(preview.provisioningProfile?.provisionedDevices == ["DEVICE-1", "DEVICE-2"], "provisioned devices")
        ...
        assert(preview.provisioningProfile?.certificates.first?.organization == "Preview Team", "certificate organization")
        assert(preview.provisioningProfile?.certificates.first?.countryName == "US", "certificate country")
        assert(preview.provisioningProfile?.certificates.first?.notBefore != nil, "certificate not before")
        assert(preview.provisioningProfile?.certificates.first?.notAfter != nil, "certificate not after")
