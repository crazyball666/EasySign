...
@implementation MachOSignatureResult
@end
...
+ (NSArray<MachOSignatureResult *> *)loadSignature: (NSURL *)filePath {
    NSMutableArray<MachOSignatureResult *> *ret = [[NSMutableArray alloc] init];
    NSArray *list = (__bridge_transfer NSArray *)load_code_signatures(filePath.path.UTF8String);
    [list enumerateObjectsUsingBlock:^(NSDictionary*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        MachOSignatureResult *item = [[MachOSignatureResult alloc] init];
        item.arch = obj[@"ARCH"];
        item.entitlements = [[NSString alloc] initWithData:obj[@"Entitlements"] encoding:NSUTF8StringEncoding];
        [ret addObject:item];
    }];
    return ret;
}
