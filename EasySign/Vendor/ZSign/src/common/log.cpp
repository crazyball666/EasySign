...
ZLog::LogCallback ZLog::g_callback = NULL;
void* ZLog::g_callbackContext = NULL;

void ZLog::SetLogCallback(LogCallback callback, void* pContext)
{
	g_callback = callback;
	g_callbackContext = pContext;
}
...
void ZLog::_Print(const char* szLog, int nColor)
{
	...
	if (NULL != g_callback) {
		...
		g_callback(nLevel, szLog, g_callbackContext);
		return;
	}
